    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      {products.map((product) => (
        <div key={product.id} className="bg-white border border-gray-200 rounded-lg shadow hover:shadow-lg transition-shadow p-6 flex flex-col items-center">
          {/* Ürün resmi */}
          <div className="w-28 h-28 mb-4 rounded overflow-hidden">
            {product.image_url ? (
              <img 
            ...
            <div className="w-full h-full bg-gray-200 rounded flex items-center justify-center text-gray-400 text-lg font-bold" style={{ display: product.image_url ? 'none' : 'flex' }}>
              Resim
            </div>
          </div>
